...
public class EasyEsTest {
    ...
    @Resource
    DocumentEsMapper documentEsMapper;

    @Test
    public void testCreateIndex() {
        // 注意：easy-es.global-config.process-index-mode: manual
        // 测试创建索引,框架会根据实体类及字段上加的自定义注解一键帮您生成索引 需确保索引托管模式处于manual手动挡(默认处于此模式),若为自动挡则会冲突
        boolean success = documentEsMapper.createIndex();
    ...
    public void testSelect() {
        ...
        String title = "老汉";
        Document document = EsWrappers.lambdaChainQuery(documentEsMapper)
    ...
    public void testSearch() {
        // 测试查询 写法和MP一样 可以用链式,也可以非链式 根据使用习惯灵活选择即可
        List<Document> document = EsWrappers.lambdaChainQuery(documentEsMapper)
                .like(Document::getContent, "技术硬")
                .list();
        System.out.println(document);
    }
    ...
    public void testUpdate() {
        ...
        documentEsMapper.update(document2, wrapper);
    ...
    public void testDelete() {
        ...
        int successCount = documentEsMapper.delete(wrapper);
        System.out.println(successCount);
    }
